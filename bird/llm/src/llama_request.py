...
def connect_llama(engine, prompt, max_tokens, temperature, stop):
    # print(f"engine={engine}, prompt={prompt}, max_tokens={max_tokens}, temperature={temperature}, stop={stop})")
    try:
        ...
        if engine == "meta-llama/Llama-3.3-70B-Instruct-Turbo":
            llm = llama33_70b
        else:
            llm = llama31_405b
            print("using Llama 405b")
...
def collect_response_from_gpt(
    db_path_list, question_list, api_key, engine, knowledge_list=None
):
    """
    :param db_path: str
    :param question_list: []
